...
void LonSensors_c::handleDataHig(LonDevice_c* dev_p, uint8_t portNo, uint8_t* sensorData)
{
 //printf("rec HIgro data, port=%d\n",portNo);
  ...
  if((portNo == 0) | (portNo == 1))
  {
    ...
    sig_p->port = portNo;
    sig_p->data[0] = ((LonPortDataHigro_c*)dev_p->GetPortData(portNo))->GetSensorValue(0);
    sig_p->data[1] = ((LonPortDataHigro_c*)dev_p->GetPortData(portNo))->GetSensorValue(1);
...
void LonSensors_c::handleDataPress(LonDevice_c* dev_p, uint8_t portNo, uint8_t* sensorData)
{
  ...
  {  
    ...
    sig_p->lAdr = dev_p->GetLAdr();
    sig_p->port = portNo;
    sig_p->data[0] = ((LonPortDataPress_c*)dev_p->GetPortData(portNo))->GetSensorValue(0);
    sig_p->type = 1;
    sig_p->Send();
    #endif
  }
...
void LonRainSensor_c::HandleEvent(int idx)
{
  if(idx < NO_OF_RAIN_SENSORS)
  {

    rainSensor[idx].accRain++;
  }
  

}

void LonRainSensor_c::SendAcumulatedData(bool fullHourIndicator)
{
  for(int idx =0;idx< NO_OF_RAIN_SENSORS; idx++)
  {
    ...
    sig_p->lAdr = 0;
    sig_p->port = 0;
    sig_p->data[0] = sensor_p->scale * sensor_p->accRain;
    sig_p->type = 3;
    sig_p->Send();
    #endif
    ...
    sensor_p->lastHour[sensor_p->lastHourIdx] = sensor_p->accRain;
    sensor_p->lastHourIdx++;
    if(sensor_p->lastHourIdx >= 12) { sensor_p->lastHourIdx = 0; }
    ...
    {  
      ...
      for(int i=0;i<12;i++)
      {
        hourSum += sensor_p->lastHour[i];
      }

      sensor_p->lastHours[sensor_p->lastHoursIdx] = hourSum;
      sensor_p->lastHoursIdx++;
      if(sensor_p->lastHoursIdx >= 12) { sensor_p->lastHoursIdx = 0; }
    }
    sensor_p->accRain = 0;
...
void LonRainSensor_c::GetRainStats(LonGetRainStats_c* recSig_p)
{
  if(recSig_p->idx < NO_OF_RAIN_SENSORS)
  {
    LonRainSensor_c* sensor_p = &rainSensor[recSig_p->idx];

    recSig_p->scale = sensor_p->scale;

    recSig_p->current = sensor_p->accRain;

    int idx = sensor_p->lastHourIdx;
    for(int i=0;i<12;i++)
    {
      idx--;
      if(idx <0) { idx = 11; }

      recSig_p->lastHour[i] = sensor_p->lastHour[idx];
    }

    idx = sensor_p->lastHoursIdx;
    for(int i=0;i<48;i++)
    {
      idx--;
      if(idx <0) { idx = 47; }

      recSig_p->lastHours[i] = sensor_p->lastHours[idx];
    }
