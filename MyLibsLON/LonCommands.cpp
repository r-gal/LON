...
comResp_et Com_configdevice::Handle(CommandData_st* comData)
{
  ...
  if(lAdrValid)
  {
    ...
    #if USE_EXTERNAL_LINK == 1
    GetDevConfigAck_c* sig2_p =  new GetDevConfigAck_c;
    sig2_p->lAdr - lAdr;
    sig2_p->reason = 1;
    sig2_p->Send();
    #endif
...
comResp_et Com_settimer::Handle(CommandData_st* comData)
{
  ...
  if(idxValid)
  {
    ...
    #if USE_EXTERNAL_LINK == 1
    GetTimDataAck_c* sig_p = new GetTimDataAck_c;
    sig_p->timIdx = idx;
    sig_p->reason = 1;
    sig_p->Send();
    #endif
...
comResp_et Com_rainsensor::Handle(CommandData_st* comData)
{
  ...
  sprintf(textBuf," Rain sensor data \n");
  Print(comData->commandHandler,textBuf);

  sprintf(textBuf,"Act: %.2f LastHour: %.2f, last 24H: %.2f last 48H: %.2f \n",
   sig_p->scale * sig_p->current,
   sig_p->scale * lastHour,
   sig_p->scale * last24Hours,
   sig_p->scale * last48Hours
  ...
  for(int idx=0; idx<12;idx++)
  {
    sprintf(textBuf,"%d min: %.2f\n", 5*idx, sig_p->scale * sig_p->lastHour[idx] );
    Print(comData->commandHandler,textBuf);
  }
